    ...
    W: Persist<Target = Wallet>,
{
    ...
    pub async fn make_benchmark_chains(
        ...
        for (key_pair, chain_client) in key_pair_and_chain_client {
            join_set.spawn(async move {
                ...
                if !ownership.owners.is_empty() || ownership.super_owners.len() != 1 {
                    return Ok::<Option<(ChainId, KeyPair)>, Error>(None);
                }
                Ok::<Option<(ChainId, KeyPair)>, Error>(Some((chain_client.chain_id(), key_pair)))
            });
        }
        ...
        while let Some(result) = join_set.join_next().await {
            ...
            if let Some((chain_id, key_pair)) = result.unwrap()? {
                benchmark_chains.insert(chain_id, key_pair);
            }
        }
        ...
        for i in (0..num_chains_to_create).step_by(operations_per_block) {
            ...
            join_set.spawn(async move {
                Self::execute_open_chains_operations(
                    num_new_chains,
                    chain_client,
                    balance,
                    key_pair,
        ...
        for result in results {
            let (certificate, num_new_chains, key_pair) = result?;
    ...
    async fn execute_open_chains_operations(
        ...
        key_pair: KeyPair,
        admin_id: ChainId,
    ) -> Result<(ConfirmedBlockCertificate, usize, KeyPair), Error> {
